...
public class SchedulerUtil {
    ...
    public static void runTaskTimer(Plugin plugin, Runnable task, long delayTicks, long periodTicks) {
        runTaskTimerCancellable(plugin, task, delayTicks, periodTicks);
    }

    /**
     * Run a repeating task and return a cancellation handle.
     */
    public static TaskHandle runTaskTimerCancellable(Plugin plugin, Runnable task, long delayTicks, long periodTicks) {
        if (IS_FOLIA) {
    ...
    public static void runTaskTimerAsync(Plugin plugin, Runnable task, long delayTicks, long periodTicks) {
        runTaskTimerAsyncCancellable(plugin, task, delayTicks, periodTicks);
    }

    /**
     * Run a repeating async task and return a cancellation handle.
     */
    public static TaskHandle runTaskTimerAsyncCancellable(Plugin plugin, Runnable task, long delayTicks, long periodTicks) {
        if (IS_FOLIA) {
        ...
        BukkitTask scheduled = Bukkit.getScheduler().runTaskTimerAsynchronously(plugin, task, delayTicks, periodTicks);
        return scheduled::cancel;
    }
